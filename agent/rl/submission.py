...
def make_grid_map(board_width, board_height, beans_positions:list, snakes_positions:dict, agents_index):
    snakes_map = [[[0] for _ in range(board_width)] for _ in range(board_height)]
    snakes_rep = [[0 for _ in range(board_width)] for _ in range(board_height)]
    beans_rep = [[0 for _ in range(board_width)] for _ in range(board_height)]
    #todo: shortest path map 3dim each one represents the shortest path to the nearst bean
    #todo: bean distance map
    for index, pos in snakes_positions.items():
        for p in pos:
            snakes_map[p[0]][p[1]][0] = index
            if index in agents_index:
                snakes_rep[p[0]][p[1]] = 1
            else:
                snakes_rep[p[0]][p[1]] = 2 #relu cnn? 
    for bean in beans_positions:
        snakes_map[bean[0]][bean[1]][0] = 1
        beans_rep[bean[0]][bean[1]] = 1
    
    return snakes_map, [snakes_rep, beans_rep]
...
def get_observations(state, agents_index, obs_dim, height, width):
    ...
    snake_map, [snakes_rep, beans_rep] = make_grid_map(board_width, board_height, beans_positions, snakes_positions, agents_index)
    ...
    return #observations #[snakes_rep, beans_rep]
